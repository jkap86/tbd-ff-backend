/**
 * Record Service (Refactored with Dependency Injection)
 *
 * This is an example refactoring of recordService.ts showing how to use DI.
 * This file is for reference only - not meant to replace the current implementation yet.
 *
 * Key Changes:
 * 1. Class-based service instead of standalone functions
 * 2. Dependencies injected via constructor with defaults
 * 3. Uses injected db and logger instead of imports
 * 4. Maintains backward compatibility via singleton export
 */
...
export class RecordService {
  ...
  async finalizeWeekScores(
    ...
    try {
      ...
      if (!weekComplete) {
        this.logger.info(`[FinalizeScores] Week ${week} is not complete yet, skipping finalization`);
        return;
      }

      this.logger.info(`[FinalizeScores] Week ${week} is complete, finalizing scores...`);
      ...
      `;
      const matchupsResult = await this.db.query(matchupsQuery, [
        leagueId,
        week,
        season,
      ]);
      ...
      this.logger.info(`[FinalizeScores] Found ${matchups.length} matchups to finalize`);
      ...
      const rostersQuery = `
        ...
        WHERE id = ANY($1::int[])
      `;
      const rostersResult = await this.db.query(rostersQuery, [Array.from(rosterIds)]);
      const rostersMap = new Map(
        rostersResult.rows.map((r: any) => [r.id, r.settings || {}])
      );
      ...
      if (rosterUpdates.size > 0) {
        ...
        await this.db.query(batchUpdateQuery, [
          rosterIdsList,
          winsValues,
          lossesValues,
          tiesValues,
      ...
      const batchMatchupUpdateQuery = `
        ...
        WHERE id = ANY($1::int[])
      `;
      await this.db.query(batchMatchupUpdateQuery, [matchupIds]);

      this.logger.info(`[FinalizeScores] Successfully finalized week ${week} scores`);
    } catch (error) {
  ...
  async resetAllRosterRecords(leagueId: number): Promise<void> {
    try {
      ...
      await this.db.query(query, [leagueId]);
      this.logger.info(`[ResetRecords] Reset all roster records for league ${leagueId}`);
    } catch (error) {
      this.logger.error("Error resetting roster records:", { error });
      throw error;
    }
  }
  ...
  async recalculateAllRecords(
    ...
    try {
      ...
      await this.resetAllRosterRecords(leagueId);
      ...
      `;
      await this.db.query(resetMatchupsQuery, [leagueId, season]);
      ...
      const matchupsQuery = `
        ...
        ORDER BY week ASC
      `;
      const matchupsResult = await this.db.query(matchupsQuery, [leagueId, season, startWeek]);
      const matchups = matchupsResult.rows;
      ...
      if (matchups.length === 0) {
        this.logger.info(`[RecalculateRecords] No matchups to process`);
        return;
      }
      ...
      // Batch update all matchups as finalized
      const matchupIds = matchups.map((m: any) => m.id);
      const batchMatchupUpdateQuery = `
        UPDATE matchups
        SET finalized = TRUE
      ...
      this.logger.error("Error recalculating records:", { error });
      throw error;
    }
  }
}
...
export const resetAllRosterRecords = recordService.resetAllRosterRecords.bind(recordService);
export const recalculateAllRecords = recordService.recalculateAllRecords.bind(recordService);

// Export the class as default for DI usage
